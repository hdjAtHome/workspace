CREATE procedure [etl].[GenerelleDrivereTogførerFR] @model Varchar(1) = 'R', @Periode Varchar(50)
as
begin
...
'SELECT ''' + driverNavn + ''', ''CostObject'' As DestModuleType, ' 
   ...
   + 'ISNULL(SUM(Timer), 0), NULL as FixedValue, ''' + @Periode + ''' FROM dbo.GD_' + @model + '_Togførertid_FR '
...
from dbo.MD_G_KRIT_DriverSys_Togførertid_FR
where periode = @Periode

-- Fjern eksisterende
set @SQL_command = 'DELETE FROM dbo.ABC_' + @model + '_DR_Togførertid_FR WHERE Periode = ''' + @Periode + ''''
execute (@SQL_command)
...
begin
  ...
  set @SQL_command = 
  'INSERT INTO [dbo].[ABC_' + @model + '_DR_Togførertid_FR](DriverName, DestModuleType, DestReference, 
