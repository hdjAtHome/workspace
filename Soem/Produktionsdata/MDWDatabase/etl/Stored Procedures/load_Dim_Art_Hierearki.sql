...
CREATE PROC [etl].[load_Dim_Art_Hierearki]
as 
BEGIN
/*
Initialt:
...
(	SELECT  
	...
	SELECT  
		Art as ArtId
	FROM 
		ods.md_artshierarki_manuelle_tilføjelser
) A
...
TRUNCATE TABLE [edw].[Dim_Art_Hierarki]

--Indsæt arter fra Dirk
INSERT INTO [edw].[Dim_Art_Hierarki] (
	[Pk_key],
	[Art],
	[Artsnavn],
	[Parent_Key],
...
SELECT DISTINCT 
	...
	a.parent
FROM 
	(select Art as ArtId, Artsnavn as MemberName, Parent from ods.md_artshierarki_manuelle_tilføjelser
		except
	select ArtID,MemberName,Parent from edw.md_art_dim) a
		left outer join
	ods.key_Dim_Art_Hierarki k on a.artid = k.art
		left outer join
	ods.key_Dim_Art_Hierarki parentK on a.parent = parentK.art
